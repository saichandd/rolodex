...
int nbuffconcat (buffer,n,s1,s2,s3,s4,s5,s6)
...
int nocase_compare (s1,l1,s2,l2)
...
void sreverse (buffer,astring) char *buffer, *astring;
{
...

{
  ...
  while (T) {
    ...
    switch (len = get_line(iport,buffer,20)) {
      case (AT_EOF) :
        return(ANSWER_EOF);
        break;
      case (TOO_MANY_CHARS) :
...
int get_line (iport,buffer,buflen) FILE *iport; char *buffer; int buflen;
  ...
  /* if successful get_line returns the number of characters read exclusive */
...

{
  int len;
  char *line;
  if (0 > (len = get_line(fp,linebuf,maxlinelen))) {
     if (len == AT_EOF) {
...
extern int read_file_into_buffer (
  ...
  /* reads lines using get_line and stores them into buffer, terminating each */
...

{
  ...
  while (T) {
    ...
    if (0 > (len = get_line(fp,linebuffer,linebuflen))) {
       fclose(fp);
       if (len == AT_EOF) {
          return(linecount);
       }
